...
">
    ...
    <form action="{{ route('trabajadores.update', $trabajador->id) }}" method="POST">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px; margin-bottom: 30px;">
            ...
            <div>
                ...
                <select name="tipo" required 
                    ...
                    <option value="asesor" {{ old('tipo', $trabajador->tipo) == 'asesor' ? 'selected' : '' }}>Asesor</option>
                    <option value="operario" {{ old('tipo', $trabajador->tipo) == 'operario' ? 'selected' : '' }}>Operario</option>
                </select>
                @error('tipo')
                    <p style="color:#f66; margin-top:8px;">{{ $message }}</p>
                @enderror
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:30px;">
                    <div>
                        ...
                        <input type="text" name="codigo_vendedor" 
                               value="{{ old('codigo_vendedor', $trabajador->codigo_vendedor ?? '') }}"
                               placeholder="Código vendedor"
                               style="width:100%; padding:16px; border-radius:15px; background:rgba(255,255,255,0.15); border:none; color:white; font-size:16px;">
                    </div>
                    <div>
                        <label style="color: var(--primary); display:block; margin-bottom:8px;">Comisión %</label>
                        <input type="number" name="comision_porcentaje" 
                               value="{{ old('comision_porcentaje', $trabajador->comision_porcentaje ?? '') }}"
                               placeholder="Comisión %"
                               style="width:100%; padding:16px; border-radius:15px; background:rgba(255,255,255,0.15); border:none; color:white; font-size:16px;">
                    </div>
                    <div>
                        <label style="color: var(--primary); display:block; margin-bottom:8px;">Meta mensual en cubos</label>
                        <input type="number" name="meta_mensual" 
                               value="{{ old('meta_mensual', $trabajador->meta_mensual ?? '') }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="zona_asignada" 
                               value="{{ old('zona_asignada', $trabajador->zona_asignada ?? '') }}"
                               placeholder="Zona asignada"
                               style="width:100%; padding:16px; border-radius:15px; background:rgba(255,255,255,0.15); border:none; color:white; font-size:16px;">
                    </div>
                    <div>
                        <label style="color: var(--primary); display:block; margin-bottom:8px;">Tipo comisión</label>
                        <input type="text" name="tipo_comision" 
                               value="{{ old('tipo_comision', $trabajador->tipo_comision ?? '') }}"
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:30px;">
                    ...
                    <div>
                        ...
                        <input type="text" name="licencia" 
                               value="{{ old('licencia', $trabajador->licencia ?? '') }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="turno" 
                               value="{{ old('turno', $trabajador->turno ?? '') }}"
                               placeholder="Turno"
                               style="width:100%; padding:16px; border-radius:15px; background:rgba(255,255,255,0.15); border:none; color:white; font-size:16px;">
                    </div>
                    <div>
        ...
        <div style="text-align:center; margin-top:50px;">
            <button type="submit" 
                    class="btn-login" 
                    style="padding:18px 60px; font-size:20px; width:100%; max-width:420px; background:var(--primary); color:white; border:none; border-radius:15px;">
        ...
        <div style="display:flex; justify-content:flex-start; margin-top:40px;">
            <a href="{{ route('trabajadores.index') }}" 
               style="padding:18px 50px; font-size:20px; background:rgba(255,255,255,0.15); color:var(--primary); border:2px solid var(--primary); border-radius:12px; text-decoration:none; display:flex; align-items:center; gap:10px;">
